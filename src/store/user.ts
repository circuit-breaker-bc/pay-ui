...
export const useUserStore = defineStore('user', () => {
  ...
  function setAffidavitDoc (affidavitDoc: File) {
    state.affidavitDocId = '' // reset the file id
    state.affidavitDoc = affidavitDoc
  }

  function setNotaryInformation (notaryInformation: NotaryInformation) {
    state.notaryInformation = JSON.parse(JSON.stringify(notaryInformation))
  }
  ...
  async function uploadPendingDocsToStorage () {
    const isPendingUpload = !state.affidavitDocId
    if (isPendingUpload) {
  ...
  async function saveUserTerms () {
    const response = await UserService.updateUserTerms('@me', termsOfUseVersion.value, isTermsAccepted.value)
    if (response?.data) {
      return response.data
    }
  }
