...
<template>
  <div class="bg-white fas-search">
    <div class="flex flex-wrap justify-between px-4 py-3.5 border-b border-accented">
      <div class="flex">
        <UIcon name="i-mdi-view-list" class="mr-2 size-6" style="margin-top: 5px;"/>
        <h2>
          Search Routing Slip
        </h2>
      </div>      
      ...
      >
        <template #body-top>
          <tr class="sticky-row header-row-2">
            <th class="text-left px-2 py-2" v-if="columnVisibility.routingSlipNumber">
              <UInput v-model="filters.routingSlipNumber" placeholder="Routing Slip Number" size="md" class="pt-0" @input="debouncedSearch()"/>
            </th>
            <th class="text-left px-2 py-2" v-if="columnVisibility.receiptNumber">
              <UInput v-model="filters.receiptNumber" placeholder="Receipt Number" size="md" class="pt-0" @input="debouncedSearch()"/>
            </th>
            <th class="text-left px-2 py-2" v-if="columnVisibility.accountName">
              <UInput v-model="filters.accountName" placeholder="Entity Number" size="md" class="pt-0" @input="debouncedSearch()"/>
            </th>
            <th class="text-left px-2 py-2" v-if="columnVisibility.createdName">
              <UInput v-model="filters.createdName" placeholder="Created By" size="md" class="pt-0" @input="debouncedSearch()"/>
            </th>
            <th class="text-left px-2 py-2 date" v-if="columnVisibility.date">
              <DateRangeFilter v-model="filters.dateFilter" @change="search()"/>
            </th>
            <th class="text-left px-2 py-2" v-if="columnVisibility.status">
              <status-list
                column="status"
            ...
            <th class="text-left px-2 py-2" v-if="columnVisibility.refundStatus">
              <status-list
                column="refundStatus"
                class="text-input-style "
            ...
            <th class="text-left px-2 py-2" v-if="columnVisibility.businessIdentifier">
              <UInput v-model="filters.businessIdentifier" placeholder="Reference Number" size="md" class="pt-0" @input="debouncedSearch()"/>
            </th>
            <th class="text-left px-2 py-2" v-if="columnVisibility.chequeReceiptNumber">
              <UInput v-model="filters.chequeReceiptNumber" placeholder="Cheque  Number" size="md" class="pt-0" @input="debouncedSearch()"/>
            </th>
            <th class="text-left px-2 py-2" v-if="columnVisibility.remainingAmount">
              <UInput v-model="filters.remainingAmount" placeholder="Balance" size="md" class="pt-0" @input="debouncedSearch()"/>
            </th>
            <th class="text-right pl-2 pr-4 sticky data-[pinned=left]:left-0 data-[pinned=right]:right-0" data-pinned="right">
              <UButton 
                label="Clear Filters" 
                variant="outline"
